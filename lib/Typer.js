export default class Typer {
    ...
    async processLine(line) {
        ...
        me.setCurrentLineVisible(true);
        line.classList.add('b-current-line');
        ...
        for (let node = treeWalker.currentNode; node && line.contains(node); node = treeWalker.nextNode()) {
            ...
            if (node.nodeType === Element.TEXT_NODE || nodeText === '') {
                const isMetaNode = line.dataset.pauseAfter;
    ...
    setCurrentLineVisible(visible) {
        this.onLineVisibilityChange?.(this.currentLineIndex, visible);
    }
