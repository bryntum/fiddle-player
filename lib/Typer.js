export default class Typer {
    ...
    async start() {
        const
            me    = this,
            lines = Array.from(me.codeElement.children);
        ...
        for (let i = me.currentLineIndex; i < lines.length; i++) {
            ...
            if (line.dataset.goto) {
                ...
                i = parseInt(line.dataset.goto, 10) - 2;
            }
        }
    ...
    async typeChars(textNode) {
        ...
        if (textNode.nodeType === Element.TEXT_NODE) {
            textNode.parentElement.classList.add('b-current');
        }
        else {
            // empty span (=== empty row)
            textNode.classList.add('b-current');
        }
