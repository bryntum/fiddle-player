export default class Typer {
    ...
    async processLine(line) {
        ...
        for (let node = treeWalker.currentNode; node && line.contains(node); node = treeWalker.nextNode()) {
            ...
            if (node.nodeType === Element.TEXT_NODE || nodeText === '') {
                const
                    comment = nodeText.match(/\/\/\s*(.*)/),
                    commentText = comment?.[1],
                    pause = commentText?.match(/\.\.\.?(\d+)/),  // ..2  ...2
                    meta = pause;

                if (meta) {
                    node.textContent = nodeText.substr(0, comment.index);
                }
                ...
                if (meta) {
                    ...
                    if (pause) {
                        const seconds = parseInt(pause[1], 10);
                        await me.wait(seconds * 1000);
                    }

                    // If this was a pure meta-row, hide
                    if (comment.index === 0) {
                        me.setCurrentLineVisible(false);
                    }
                }
            }
        }
    ...
    setCurrentLineVisible(lineIndex, visible) {
        this.onLineVisibilityChange?.(this.currentLineIndex, false);
    }
    ...
    async typeChars(textNode) {
        ...
        for (let i = startPosition; i <= text.length; i++) {
            if (textNode.nodeType === Element.TEXT_NODE && textNode.parentElement.offsetLeft + textNode.parentElement.offsetWidth > this.codeElement.offsetWidth - 40) {
