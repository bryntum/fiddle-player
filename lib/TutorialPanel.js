    ...
    highlightCode(codeSnippet) {
        ...
        return codeSnippet.split('\n').map(line => {
            const
                ...
                rowDataset        = {},
                trimmed           = line.trim(),
                rawTrimmed        = trimmed.split('///')[0],
                commentStartParts = rawTrimmed.split('/*'),
                commentEndParts   = rawTrimmed.split('*/');

            let trimmedLine       = commentStartParts.length > 0 ?
                    commentStartParts[0] :
                    (commentEndParts.length > 0 ? commentEndParts[1] : rawTrimmed),
            ...
            let inlineCommentParts = trimmedLine.split('//');
            ...
            const lineChildren = trimmedLine ? trimmedLine.split(' ').map((word, index, all) => {
                let matchedWord;
                const
                    children = [];
