...
export default class TutorialPanel {
        ...
        return `
            <div class='code-wrap' style='${this.codeWrapStyle || ''};'>
                <div class="b-top-toolbar" data-reference="topToolbar">
                    ...
                    <svg data-reference="darkButton" data-action="darkTheme" height="16" version="1.1" viewBox="0 0 16 16" width="16">
                        <path d="M6 .278a.768.768 0 0 1 .08.858 7.208 7.208 0 0 0-.878 3.46c0 4.021 3.278 7.277 7.318 7.277.527 0 1.04-.055 1.533-.16a.787.787 0 0 1 .81.316.733.733 0 0 1-.031.893A8.349 8.349 0 0 1 8.344 16C3.734 16 0 12.286 0 7.71 0 4.266 2.114 1.312 5.124.06A.752.752 0 0 1 6 .278zM4.858 1.311A7.269 7.269 0 0 0 1.025 7.71c0 4.02 3.279 7.276 7.319 7.276a7.316 7.316 0 0 0 5.205-2.162c-.337.042-.68.063-1.029.063-4.61 0-8.343-3.714-8.343-8.29 0-1.167.242-2.278.681-3.286z"></path>
                    </svg>
                    <svg data-reference="lightButton" data-action="lightTheme" height="16" version="1.1" viewBox="0 0 16 16" width="16">
                    ...
                    <svg data-action="toggleFullscreen" height="16" version="1.1" viewBox="0 0 16 16" width="16">
    ...
    async toggleTheme(theme) {
        const
            { element } = this,
            dark        = theme === 'dark',
            outerEl     = this.element.getRootNode().host;

        element.classList.toggle('b-dark', dark);
        this.lightButton.classList.toggle('pressed', !dark);
        this.darkButton.classList.toggle('pressed', dark);

        const href = element.getRootNode().querySelector('[data-bryntum-theme]').href;

        element.getRootNode().querySelector('[data-bryntum-theme]').href = href.replace(dark ? 'light' : 'dark', dark ? 'dark' : 'light');

        if (this.constructed) {
            element.classList.add('b-theme-changing');
            setTimeout(() => element.classList.remove('b-theme-changing'), 5000)
        }
    }
    ...
    async evaluateCode(ignoreError) {
        ...
        try {
            ...
            if (globalThis.bryntum) {
                ...
                if (widget) {
                    if (widget.features.projectLines) {
                        widget.features.projectLines.disabled = true;
                    }
                    if (widget.taskStore) {
                        widget.taskStore.modelClass.constructor.convertEmptyParentToLeaf = false;
                        widget.taskStore.syncDataOnLoad = widget.taskStore.count > 1;
                        widget.eventColor = 'blue';
                        widget.transitionDuration = 500;
                    }
                    const
                    ...
                    try {
                        ...
                        if (widget.features?.dependencies) {
                            widget.features.dependencies.radius = 10;
                        }
                        if (widget.features?.projectLines) {
                            widget.features.projectLines.disabled = true;
                        }
    ...
    async loadResources() {
        if (!this.resourcesLoaded && !this.loadingResources) {
            ...
            const
                ...
                faPath       = (!isChrome || !document.fonts.check(`normal 14px "Font Awesome 6 Free"`)) && 'https://bryntum.com/dist/gantt/resources/fontawesome/webfonts/',
                stylesheets  = me.dataset.stylesheets,
            ...
            stylesheets.split(',').forEach(url => {
                ...
                if (/visby/.test(url)) {
                    link.dataset.bryntumTheme = 'true';
                }
                // Load each stylesheet and wait for it to be loaded
            ...
            font = new FontFace(isFF ? 'FontAwesome6Free' : 'Font Awesome 6 Free', `url("https://bryntum.com/dist/gantt/resources/fontawesome/webfonts/fa-solid-900.woff2")`);
