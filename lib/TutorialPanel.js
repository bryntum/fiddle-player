...
export default class TutorialPanel {
        ...
        return `
            <div class='code-wrap'>
                <pre data-reference='pre'>
                    ...
                    <code class='b-idle' contenteditable spellcheck='false' data-reference='codeElement'></code>
                </pre>
                <div class='b-code-toolbar' data-reference='toolbar'>
                    <div class='run action' data-btip='Run code' data-action='run'><i style="margin-right:0.8em" class="b-icon b-fa-play"></i>Run</div>
                    <i class='b-icon b-fa b-fa-circle-notch b-fa-spin' data-reference='loadingIcon'></i>
                    <i class='b-icon b-fa-exclamation-triangle'  data-reference='errorIcon'></i>
                    <div class='action' data-action='increase-fontsize'>A<i style="margin-left:0.3em" class="b-icon b-fa-caret-up"></i></div>
                    <div class='action' data-action='decrease-fontsize'><small>A</small><i style="margin-left:0.3em" class="b-icon b-fa-caret-down"></i></div>
                    <i class='action b-icon b-icon-copy' data-btip='Copy to clipboard' data-action='copy'></i>
                    <span class='action b-line-count' data-btip='Copy to clipboard' data-action='copy' data-reference='progressElement'></span>
                </div>
            </div>
            <div class="b-codepanel-splitter" data-reference="splitter"></div>
            <div class='b-codepanel-result' data-reference="resultContainer"></div>
       `;
    }
    ...
    async construct(config) {
        ...
        me.pre.addEventListener('click', me.onPreClick.bind(me));
        ...
        if (me.url) {
            ...
            me.codeElement.classList.add('b-editable');
            me.toolbar.style.display = 'none';
            me.codeElement.focus();
        }

        me.typer = new Typer(Object.assign({
            codeElement        : me.codeElement,
            keystrokeInterval  : config.keystrokeInterval || 20,
            intervalRandomness : config.intervalRandomness || 10,
    ...
    highlightCode(codeSnippet) {
        return codeSnippet.split('\n').map(line => {
            ...
            const lineChildren = code && !comment ? code.split(' ').map((word, index, all) => {
                let matchedWord;
                const
                    children = [];
        ...
        switch (actionNode.dataset.action) {
            ...
            case 'run':
                if (this.isTyping) {
                    ...
                    if (this.typer.currentLineIndex === this.lineCount) {
    ...
    async evaluateCode(ignoreError) {
        ...
        this.element.classList.remove('b-error');
        ...
        try {
            ...
            imports?.forEach(importLine => {
                const
                    parts = !importLine.includes('http') && importLine.split('../');
            ...
            if (imports?.length && !me.processedImports) {
                me.element.classList.add('b-loading');
            }
            ...
            delete this.resultContainer.dataset.error;
        }
        catch (e) {
            if (this.isTyping && (e instanceof SyntaxError || e.message.startsWith('Bryntum bundle'))) {
                ...
                this.element.classList.add('b-error');
                this.errorIcon.dataset.btip = e.message;
            }
        }
...
class TutorialPanelTag extends HTMLElement {
    ...
    async setup() {
        ...
        const
            ...
            faPath     = (!isChrome || !document.fonts.check(`normal 14px "Font Awesome 5 Free"`)) && me.getAttribute('fa-path'),
            ...
            link       = me.linkTag = Object.assign(document.createElement('link'), {
                tag  : 'link',
                rel  : 'stylesheet',
                href : stylesheet
            }),
    ...
    destroy() {
        ...
        me.linkTag.remove();
        this.internalStyleSheet.remove();
